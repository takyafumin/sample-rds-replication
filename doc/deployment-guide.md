...
- DMSに必要なIAMロールが設定されていること（詳細は[DMS IAMロールの設定](dms-iam-setup.md)を参照）
...
- VPCとサブネット
- 2つのAurora MySQLクラスター（ターゲットDBとソースDB）
- 踏み台サーバー用のEC2インスタンス
...
### ステップ5: DMSに必要なIAMロールを設定

DMSレプリケーションをデプロイする前に、必要なIAMロールが設定されていることを確認します。詳細な手順については、[DMS IAMロールの設定](dms-iam-setup.md)を参照してください。
...
### ステップ6: DMSレプリケーションをデプロイ

サンプルデータベースのインポートが完了し、必要なIAMロールが設定されたら、DMSレプリケーションをデプロイします。
...
このコマンドは以下のリソースをデプロイします：
- DMSレプリケーションインスタンス
- DMSソースエンドポイント（ソースDBを指す）
- DMSターゲットエンドポイント（ターゲットDBを指す）
...
1. **ポートフォワーディングの問題**: ローカルからデータベースに接続できない場合は、ポートフォワーディングが正常に設定されていることを確認してください。`./run.sh port-forward-target` と `./run.sh port-forward-source` コマンドが実行中であることを確認します。
...
7. **DMSのIAMロールエラー**: 「The IAM Role arn:aws:iam::XXXX:role/dms-vpc-role is not configured properly」などのエラーが発生した場合は、DMSに必要なIAMロールが正しく設定されていることを確認してください。詳細は[DMS IAMロールの設定](dms-iam-setup.md)を参照してください。
