...
### ステップ2: DMSに必要なIAMロールをデプロイ

DMSレプリケーションを設定する前に、必要なIAMロールをデプロイします。

```bash
./run.sh deploy-iam
```

IAMロールのデプロイ状況は以下のコマンドで確認できます：
...
サンプルデータベースのインポートとIAMロールの設定が完了したら、DMSレプリケーションをデプロイします。
...
# 3. DMSのIAMロールスタックを削除（必要な場合）
./run.sh delete-iam
```

これにより、すべてのリソースが適切に削除され、AWS上で不要な料金が発生しなくなります。

注意: IAMロールは他のAWSサービスでも使用される可能性があるため、他の環境でDMSを使用している場合は、IAMロールスタックを削除しないでください。